...
import es.upv.indigodc.alien4cloud.im.plugin.service.BuilderService;
import es.upv.indigodc.alien4cloud.im.plugin.service.EventService;
import es.upv.indigodc.alien4cloud.im.plugin.service.MappingService;
import es.upv.indigodc.alien4cloud.im.plugin.service.OrchestratorConnector;
import es.upv.indigodc.alien4cloud.im.plugin.service.model.DeploymentInfo;
import es.upv.indigodc.alien4cloud.im.plugin.service.model.OrchestratorIamException;
import es.upv.indigodc.alien4cloud.im.plugin.configuration.CloudConfiguration;
import es.upv.indigodc.alien4cloud.im.plugin.configuration.CloudConfigurationManager;
import es.upv.indigodc.alien4cloud.im.plugin.location.LocationConfiguratorFactory;
import es.upv.indigodc.alien4cloud.im.plugin.service.model.OrchestratorResponse;
import es.upv.indigodc.alien4cloud.im.plugin.service.model.StatusNotFoundException;
...
import es.upv.indigodc.alien4cloud.im.plugin.service.model.response.DeploymentOrchestrator;
import es.upv.indigodc.alien4cloud.im.plugin.service.model.response.GetDeploymentsResponse;
 ...
 */
@Slf4j
@Component("indigodc-im-plugin")
@Scope("prototype")
public class IndigoDcIMPlugin implements IOrchestratorPlugin<CloudConfiguration> {

  public final static String DEFAULT_NOT_SET_OUTPUT_VALUE = "<not_set>";
  public final static String TYPE = "IndigoDC IM Plugin";
   ...
   * {@link CloudConfiguration} instance that holds
...
//  @PreDestroy
  ...
  public void deploy(PaaSTopologyDeploymentContext deploymentContext, IPaaSCallback<?> callback) {
    ...
    try {
      ...
      log.info("Deploying on: " + configuration.getImEndpoint());
...
//          getDeploymentInfo(deploymentContext.getDeploymentPaaSId());
      if (deploymentInfo != null) {
        ...
        final OrchestratorResponse response = orchestratorConnector.callUndeploy(configuration,
...
//    }
  ...
  protected void updateStatusesDeploymentsUser(PaaSDeploymentContext deploymentContext,
    ...
    try {
      ...
      OrchestratorResponse response = orchestratorConnector.callGetDeployments(configuration);
      log.info(response.getResponse().toString());
      ...
      for (DeploymentOrchestrator deployment: deployments) {
        response = orchestratorConnector.callGetTemplate(configuration, deployment.getUuid());
        log.info(response.toString());
